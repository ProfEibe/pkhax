...
export class CommentService {
  ...
  public getCommentsByGame(id: number): Observable<Comment[]> {
    return this.http
      .get<Comment[]>(this.baseUrl + '/games/' + id + '/comments')
      .pipe(map((list) => list.map((comment) => this.adapter.adapt(comment))));
  }

  public createComment(comment: Comment): Observable<Comment> {
    return this.http
      .post(this.baseUrl + '/games/' + comment.gameId + '/comments', comment)
  ...
  public createChildComment(
    ...
    return this.http
      .post(
        this.baseUrl + '/games/' + comment.gameId + '/comments/' + parentId,
        comment,
      )
      .pipe(map((item: any) => this.adapter.adapt(item)));
  }
